...
import { computeSoilDailySummaries } from "../../../lib/soilTempMetrics";
import { useIsMobile } from "../../charts/chartUtils";

interface B01DailySoilTempChartProps {
  data: ReadingB01[];
  periodLabel: string;
}
...
export default function B01DailySoilTempChart({
  data,
  periodLabel,
}: B01DailySoilTempChartProps) {
  const isMobile = useIsMobile();

  const chartData = useMemo(() => {
    return computeSoilDailySummaries(data).map((day) => ({
      dateKey: day.dateKey,
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <div className="mb-4">
          <h3 className="text-sm font-bold text-slate-800">
            Temperatura de suelo diaria (máxima / mínima)
          </h3>
